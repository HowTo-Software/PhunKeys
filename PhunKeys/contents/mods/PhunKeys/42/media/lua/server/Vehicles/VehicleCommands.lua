print("[PhunKeys][BOOT][DEBUG2] ENTER SERVER Vehicles/VehicleCommands.lua override")
-- B42.20.2 vanilla VehicleCommands.lua, commit 8a906692ac56f9d40c078d654eea6c70491cbc62.
-- Project Zomboid copyright The Indie Stone. PhunKeys changes: require + dispatch gate.
-- Recheck against vanilla on game updates. Do not combine with another override
-- of Vehicles/VehicleCommands.lua without merging the dispatch gate.
if isClient() then return end
require "PhunKeys_CommandGuard"
...
VehicleCommands.OnClientCommand = function(module, command, player, args)
	if module == 'vehicle' and Commands[command] then
        if not PhunKeys.allowVehicleCommand(player, command, args or {}) then return end
		local argStr = ''
		args = args or {}
